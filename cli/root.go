// Package cli assembles the adviceslip command tree from the adviceslip
...
// NewApp assembles the kit application from the adviceslip domain. The
...
//
// To add a command, declare it in adviceslip/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := adviceslip.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(adviceslip.Domain{}).Register(app)
