...
describes what users get. Pick a format with `-o`, or let adviceslip choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
adviceslip <command> -o table   # aligned columns for reading
adviceslip <command> -o jsonl   # one JSON object per line, for piping
adviceslip <command> -o json    # a single JSON array
adviceslip <command> -o csv     # spreadsheet friendly
adviceslip <command> -o tsv     # tab-separated
adviceslip <command> -o url     # just the URL column
adviceslip <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
adviceslip <command> --fields id,title,url
```
...
```bash
adviceslip <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
adviceslip <command>            # a table, because this is a terminal
adviceslip <command> | wc -l    # JSONL, because this is a pipe
```
